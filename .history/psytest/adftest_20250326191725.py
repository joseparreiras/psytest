...
class BaseADFTest:
    ...
    def teststat(self, **kwargs: dict[str, Any]) -> Any:
        ...
        """
        if not hasattr(self, "__teststat__") or hasattr(self, "tstat_kwargs") or self.tstat_kwargs != kwargs:
            self.__teststat__: float64 = self.__teststat_func__(self.y, kmax = self.kmax, **kwargs)
        if not hasattr(self, "__teststat__") or self.tstat_kwargs != kwargs:
