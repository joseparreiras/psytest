...
from numpy import float64, quantile, array, sum, square, diag
...
def adfuller_stat(y: NumArray, kmax: int) -> float64:
    nobs: int = len(y)
    y_diff: NumArray = np.diff(y)
    X: NumArray = np.zeros((nobs - kmax - 1, 2 + kmax))
    X[:, 0] = np.ones(nobs - kmax - 1)  # Constant
    for k in prange(1, kmax + 1):
        X[:, k] = y_diff[kmax - k : -k]
    X[:, -1] = y[kmax:-1]
    y_diff = y_diff[kmax:]
    beta: NumArray = np.linalg.inv(X.T @ X) @ X.T @ y_diff
    fit: NumArray = X @ beta
    resid: NumArray = y_diff - fit
    ssr: float64 = np.sum(np.square(resid))
        ...
        **kwargs: dict[str, Any]
        ...
        if (
            ...
            self.__simdist__: NDArray[float64] = self.__simdist_func__(
                self.nobs, nreps=nreps, kmax=self.kmax, **kwargs, **self.kwargs
            )
