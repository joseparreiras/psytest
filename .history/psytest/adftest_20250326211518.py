...
class ADFtest:
    def __init__(self, y: NumArray, kmax: int = KMAX, **kwargs: Any) -> None:
        """
    ...
    def teststat(self) -> Any:
        ...
        if not hasattr(self, "__teststat__"):
            self.__teststat__: float64 = adfuller_stat(y=self.y, kmax=self.kmax)
    ...
    def critval(
        ...
        Args:
            ...
            kwargs (Any): Additional keyword arguments for the test statistics function.
        ...
        if (
            ...
            self.__simdist__: NumArray = adfuller_dist(
                nobs=self.nobs, nreps=nreps, kmax=self.kmax
            )
            self.critval_kwargs: dict[str, Any] = {test_size: test_size, nreps: nreps}
