...
from pytest.utils.functions import simulate_random_walks
...
@njit
def corridor_adfuller_cdf(wiener: NumArray, r1: int, r2: int) -> float:
...
def corridor_adfuller_adist(
    nobs: int, nreps: int = NREPS, index_start: int = 0, index_end: int = int(1e9)
) -> NumArray:
    index_end = min(index_end, nobs)
