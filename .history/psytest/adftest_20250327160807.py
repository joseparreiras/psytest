...
def corridor_adfuller_adist(
    nobs: int,
    nreps: int = NREPS,
    index_start: int64 = int64(0),
    index_end: int64 = int64(1e9),
) -> NumArray:
    index_end_strict: int = min(index_end, nobs) # type: ignore
    dist: NumArray = zeros(nreps)
    for j in prange(nreps):
        wiener: NumArray = random_walk(nobs)
        dist[j] = corridor_adfuller_cdf(wiener, index_start, index_end_strict)
    return dist
