...
@njit(parallel=False)
def adfuller_stat(y: NDArray[float64], kmax: int) -> float64:
    """
    ...
    coef: float64 = beta[-1]
    coef_var: float64 = sigma_sq_hat * diag(inv(X.T @ X))[-1]
...
@njit
def corridor_adfuller_cdf(wiener: NDArray[float64], i1: int64, i2: int64) -> float64:
    delta: float64 = 1 / len(wiener)
    w2: float64 = wiener[i2]
    w1: float64 = wiener[i1]
    w_sum: float64 = np.sqrt(delta) * npsum(wiener[i1:i2])
    w_sumsq: float64 = delta * npsum(wiener[i1:i2] ** 2)
    rw: float64 = (i2 - i1) / len(wiener)
