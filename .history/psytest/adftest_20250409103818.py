...
def rolling_adfuller_stat(
    y: NDArray[float64], r1: float = 0, r2: float | None = None, kmax: int = KMAX
) -> float:
    """
    ...
    Args:
        ...
        r2 (float | None, optional): End index. Defaults to None.
    ...
    """
    if (not 0 <= r1 <= 1) or ((r2 is not None) and (not 0 <= r2 <= 1)):
        raise ValueError("r1 and r2 should be in the range [0, 1]")
    nobs: int = len(y)
    i1: int = int(floor(r1 * nobs))
    i2: int = int(floor(r2 * nobs)) if r2 else 1
...
def rolling_adfuller_cdf(wiener: NDArray[float64], r1: float, r2: float) -> float:
    ...
    Args:
        wiener (NDArray[float64]): _description_
