...
def adfuller_stat(y: NDArray[float64], kmax: int) -> float:
    ...
    where :math:`\\hat{\\beta}` is the coefficient of :math:`y_{t-1}` in the regression and :math:`\\widehat{\\mathrm{Var}}(\\hat{\\beta})` is its estimated variance.
...
def rolling_adfuller_cdf(wiener: NDArray[float64], r1: float, r2: float) -> float:
    ...
    .. math::
        \\text{F}_{ADF}(r_1, r_2) = \\frac{1}{\\sqrt{r_w} \\sqrt{r_w \\cdot \\sum W^2 - (\\sum W)^2}} \\left[\\frac{1}{2} r_w (W_2^2 - W_1^2 - rw) - \\sum W (W_2 - W_1) \\right]

    where :math:`r_w = i_2 - i_1`, :math:`W_t` is the Wiener process, and the sums are over the interval :math:`[i_1, i_2]`.
