from psytest.adftest import adfuller_stat, ADFtest, NumArray, KMAX, TEST_SIZE, NREPS
from psytest.utils.functions import (
    r0_default,
    simulate_random_walks,
)
from psytest.corridor_adftest import corridor_adfuller_test
...
def calc_bsadfuller_stat(y: NumArray, r0: int, r2: int, kmax: int) -> float64:
    ...
    for r1 in prange(0, r2 - r0 + 1):
        stat = max(stat, corridor_adfuller_test(y, r1, r2, kmax))
    return stat


# Backward Sup ADF Test
def basdfuller_stat(y: NumArray, r0: int, kmax: int) -> tuple[NDArray[int64], NumArray]:
    """
    ...
    """
    nobs: int = len(y)
    r2_grid: NDArray[int64] = arange(r0, nobs + 1)
    nr2: int = len(r2_grid)
