...
class PSYBubbles:
    def __init__(self, y: NDArray[float64], r0: int | None = None, kmax: int = 0, minlength: int | None = None) -> None:
        self.y: NDArray[float64] = y
        self.nobs: int = len(y)
        self.r0: int = r0 or r0_default(self.nobs)
        self.kmax: int = kmax
        self.minlength: int = minlength or minlength_default(self.nobs)
