...
from numpy import float64, quantile, apply_along_axis, bool_
...
class PSYBubbles:
    def __init__(
        ...
        self.nobs: int = len(y)
        self.index: NDArray[Generic[IndexType]] | None = None
    ...
    def find_bubbles(self, alpha: float):
        ...
        bubble_bool: list[bool] = [
            stat[i] > cval[i][alpha_index] for i in range(len(stat))
        ]
        bubble_index: list[tuple[int, int | None]] = list(
            __find_bubble_dates(bubble_bool, self.minlength)
        )
        if hasattr(self, "index"):
            bubble_dates: list[tuple] = [()]
        return bubble_index


def __check_bubble_exists(bubble_bool: list[bool]) -> bool:
    return bubble_bool.count(True) > 0


def __check_bubble_ends(bubble_bool: list[bool]) -> bool:
    return bubble_bool.count(False) > 0


def __is_bubble_long_enough(bubble_bool: list[bool], minlength: int) -> bool:
    return bubble_bool[minlength]


def __find_bubble_dates(
    bubble_bool: list[bool], minlength: int
) -> Generator[tuple[int, int | None], None, None]:
    i0 = 0
    while len(bubble_bool) > minlength:
        if not __check_bubble_exists(bubble_bool):
            break
        start: int = bubble_bool.index(True)
        if __is_bubble_long_enough(bubble_bool[start:], minlength):
            if not __check_bubble_ends(bubble_bool[start:]):
                yield (start + i0, None)
                break
