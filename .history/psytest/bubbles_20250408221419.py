...
class PSYBubbles:
    ...
    def r2grid(self):
        return arange(self.r0, 1 + self.rstep, self.rstep)
    def teststat(self, force: bool = False) -> dict[int, float]:
        if force or not hasattr(self, "_teststat"):
            ...
            r2_grid: NDArray[float64] = arange(self.r0, 1 + self.rstep, self.rstep)
            self._teststat: dict[int, float] = dict(zip(r2_grid, stat))
