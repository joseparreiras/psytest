...
from numpy import float64, quantile, apply_along_axis, bool_, array, arange
...
class PSYBubbles:
    ...
    def find_bubbles(
        ...
        if alpha not in self._testsize:
            test_size: list[float] = [alpha]
        else:
            test_size: list[float] = self._testsize
        cval: dict[int, NDArray[float64]] = self.critval(nreps=nreps, test_size=test_size)
