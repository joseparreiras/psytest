...
class PSYBubbles:
    ...
    def find_bubbles(
        ...
        bubble_bool: list[NDArray[object_]] = [stat[i] > cval[i] for i in stat.keys()]
        bubble_r2index: NDArray[float64] = array(
            list(find_bubble_dates(bubble_bool, self.minlength))
        )
        bubble_index: NDArray[float64] = array(
            [self.r2grid()[i] for i in bubble_r2index.flatten()]
        ).reshape((-1, 2))
        bubble_nindex: NDArray = array([int(floor(self.nobs * x)) for x in bubble_index.flatten()]).reshape((-1, 2))
