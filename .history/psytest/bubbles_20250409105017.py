...
class PSYBubbles:
    ...
    def find_bubbles(self, alpha: float, nreps: int | None = None) -> NDArray[object_]:
        ...
        Raises:
            ...
            - TypeError: If `nreps` is set and is not an integer.
            - ValueError: If `nreps` is set and is less than 1.
        ...
        if nreps is not None and not isinstance(nreps, int):
            raise TypeError("`nreps` must be an integer")
        if nreps is not None and nreps < 1:
            raise ValueError("`nreps` must be greater than 0")
