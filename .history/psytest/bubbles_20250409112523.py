...
def check_bubble_exists(bubble_bool: list[bool]) -> bool:
    """
    Check for any True in the list (i.e., bubble exists).

    Parameters:
        bubble_bool (list[bool]): Indicators for rejection of the null.

    Returns:
        bool: True if any bubbles exist.
    """
...
def check_bubble_ends(bubble_bool: list[bool]) -> bool:
    """
    Check if the bubble ends (any False after start).

    Parameters:
        bubble_bool (list[bool]): Booleans from start of a bubble.

    Returns:
        bool: True if a non-bubble state is detected.
    """
...
def is_bubble_long_enough(bubble_bool: list[bool], minlength: int) -> bool:
    """
    Check if the bubble has minimum required length.

    Parameters:
        bubble_bool (list[bool]): Booleans indicating rejection.
        minlength (int): Required minimum length.

    Returns:
        bool: True if bubble is long enough.
    """
    try:
...
def find_bubble_dates(
    ...
    """
    Finds the start and end indices of bubbles.

    Parameters:
        bubble_bool (list): Rejection indicators.
        minlength (int): Minimum bubble length.

    Yields:
        tuple[int, int | None]: Start and end of each bubble.
    """
    i0 = 0
