...
def corridor_adfuller_cdf(wiener: NumArray, r2: int, r1: int) -> float64:
    w2: float64 = wiener[r2]
    w1: float64 = wiener[r1]
    w_sum: float64 = sum(wiener[r1:r2])
    w_sumsq: float64 = sum(wiener[r1:r2] ** 2)
    rw: int = r2 - r1
    return 1 / 2 * rw * (w2**2 - w1**1 - rw) - w_sum * (w2 - w1) / (rw ** 0.5 * (rw * w_sumsq - w_sum**2) ** 0.5)
