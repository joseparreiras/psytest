...
class HypothesisTest:
    def __init__(self, y: NDArray[float64], teststat_func: Callable, simdist_func: Callable) -> None:
    ...
    def critval(self, test_size: NDArray[float64], nreps: int = 1000, **kwargs: dict[str, Any]) -> NDArray[float64]:
        if not hasattr(self, "__simdist__") or len(self._simdist) != nreps:
            self.__simdist__: NDArray[float64] = NotImplemented
