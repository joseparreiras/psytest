...
@njit(parallel=True)
def calc_gsad_stat(y: np.ndarray, r0: int) -> np.float64:
    nobs: int = len(y)
    stat: int = 0
    for r2 in prange(r0, nobs):
        for r1 in prange(r0, r2 - r0):
            rw: int = r2 - r1
            num: float = 1 / 2 * rw * (y[r2]**2-y[r1]**2-rw) - sum(y[r1:r2])) * (y[r2] - y[r1])
            den: float = rw**0.5 * (rw * sum(y[r1:r2]**2) - (sum(y[r1:r2]))**2)**0.5
            stat: gloat = max(stat, num / den)
    
        
def gsadfuller_dist(random_walks: np.ndarray, r0: int) -> np.ndarray:
    nreps: int = random_walks.shape[0]
    nobs: int = random_walks.shape[1]
