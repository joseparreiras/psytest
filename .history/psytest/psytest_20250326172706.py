...
def gsadfuller_dist(nobs: int, nreps: int = 1000, r0: int) -> np.ndarray:
    random_walks: np.ndarray = simulate_random_walks(nreps, nobs)
    adf_dist: np.ndarray = np.apply_along_axis(sim_gsadf_stat, 1, random_walks, r0)
...
class GSADFuller():
    ...
    def critval(self, alpha_list: list[float] = [0.1, 0.05, 0.01], nreps: int = 1000) -> dict[float, float]:
        if not hasattr(self, "_critval"):
            random_walks: np.ndarray = simulate_random_walks(nreps, self.nobs)
            adf_dist: np.ndarray = np.apply_along_axis(sim_gsadf_stat, 1, random_walks, self._r0)
            self._critval: dict[float, float] = dict(zip(alpha_list, np.quantile(adf_dist, alpha_list)))
