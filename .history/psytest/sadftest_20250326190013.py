...
def sadfuller_dist(nobs: int, nreps: int) -> np.ndarray:
    random_walks: np.ndarray = simulate_random_walks(nobs, nreps)
    return np.apply_along_axis(sadfuller_asymp_dist, 0, random_walks, r0_default(nobs))
...
class SADFuller(HypothesisTest):
    ...
    def teststat(self, r0: int | None = None, kmax: int = KMAX) -> np.float64:
        """
        ...
        Args:
            ...
            kmax (int, optional): The maximum number of lags to include in the ADF Test. Defaults to 0.

        Returns:
            np.float64: The test statistics.
        """
        super().teststat(r0=r0, kmax=kmax)

    def critval(
        self, alpha_list: np.ndarray = TEST_SIZE, nreps: int = 1000
    ) -> np.ndarray:
