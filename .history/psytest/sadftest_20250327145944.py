from psytest.adftest import adfuller_stat, NumArray, KMAX, TEST_SIZE, NREPS, corridor_adfuller_cdf
...
from numpy import float64, inf, apply_along_axis, quantile, zeros
...
@njit(parallel=True)
def sadfuller_cdf(w: NumArray, r0: int) -> float:
    """
    ...
    for r2 in prange(r0, nobs):
        for r1 in prange(r0, r2 - r0):
            stat = max(stat, corridor_adfuller_cdf(w, r1, r2))
    return stat
...
def sadfuller_dist(nobs: int, nreps: int, r0: int) -> NumArray:
    ...
    return apply_along_axis(sadfuller_cdf, 1, random_walks, r0)

@njit(parallel=True)
def sadfuller_dist_numba(nobs: int, nreps: int, r0: int) -> NumArray:
    stat: NumArray = zeros(nreps)
    for i in range(nreps):
        rw: NumArray = random_walk(nobs)
        stat[i] = sadfuller_cdf(rw, r0)
    return stat
