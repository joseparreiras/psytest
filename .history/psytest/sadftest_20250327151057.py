...
from numpy import float64, inf, apply_along_axis, quantile, zeros, repeat
...
def __sadfuller_dist_from_random_walks__(random_walks: NDArray[float64], iterable: Iterable[tuple[int, int, int]]) -> None:
    nreps: int = len(random_walks)
    stats: NumArray = repeat(-inf, nreps)
    for (j, r2, r1) in iterable:
