...
def __sadfuller_dist_from_random_walks__(random_walks: NDArray[float64], iterable: NDArray[int64]) -> NDArray[float64]:
    assert iterable.ndim = 2
    nreps: int = len(random_walks)
    stats: NumArray = repeat(-inf, nreps)
    niter: int = len(iterable)
    for i in prange(niter):
        next_iter: NDArray[int] = iterable[i]
        current_stat: float64 = stats[j]
        next_stat: float64 = corridor_adfuller_cdf(random_walks[j], r1, r2)
