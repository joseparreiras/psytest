...
def __make_iterator__(
    ...
    j_range = range(1, nreps + 1)
    r2_range = range(r0, nobs + 1)
    r1_range = range(r0, nobs + 1)
    for j, r1, r2 in product(j_range, r1_range, r2_range):
        if r1 <= r2 - r0:
            yield j, r1, r2
...
@njit(parallel=True)
def generate_combinations(nreps: int, r0: int, nobs: int) -> np.ndarray:
    total: int = nreps * (nobs - r0 + 1) * (nobs - r0 + 1)
    result: np.ndarray = np.empty((total, 3), dtype=np.int32)

    idx: int = 0
    for n in prange(1, nreps + 1):
