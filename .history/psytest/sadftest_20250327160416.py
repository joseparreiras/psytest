...
def sadfuller_dist(nobs: int64, nreps: int64, r0: int64) -> NumArray:
    ...
    iterator: NDArray[int64] = __make_iterator(nreps, r0, nobs)
    return stat
...
@njit(parallel=False)
def __make_iterator__(nreps: int, r0: int, nobs: int) -> NDArray[int64]:
    total: int = nreps * (nobs - r0 + 1) * (nobs - r0 - r0 + 1)
