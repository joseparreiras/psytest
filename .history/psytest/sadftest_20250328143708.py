...
def bsadf_stat_all_series(
    y: NDArray[float64], r0: int64, kmax: int64
) -> tuple(NDArray[int64], NDArray[float64]):
    """
    ...
    """
    nobs: int64 = int64(len(y))
    r2r1_grid: NDArray[int64] = __r1r2_combinations__(nobs, r0)
    r2_grid: NDArray[int64] = arange(r0, nobs + 1)
    stat: NDArray[float64] = repeat(float64(-inf), len(r2_grid))
    for i in prange(len(r2r1_grid)):
        


@njit(parallel=True)
def __r1r2_combinations__(nobs: int64, r0: int64) -> NDArray[int64]:
    """
