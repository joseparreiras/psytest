...
from numpy import float64, inf, zeros, repeat, int64, arange, empty, append
...
@njit(parallel=True)
def sadfuller_dist(nobs: int, nreps: int, r0: int) -> NDArray[float64]:
    rw_array: NDArray[float64] = random_walk(nreps, nobs)
    stat: NDArray[float64] = array([], dtype=float64)
    for j in prange(nreps):
        rw: NDArray[float64] = rw_array[j]
        for r2 in range(r0, nobs):
            for r1 in range(0, r2 - r0):
                stat = append()
