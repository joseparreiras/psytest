...
from numpy import float64, inf, zeros, repeat, int64, arange, empty, array, quantile, ceil
...
def bsadf_stat_all_series(
    y: NDArray[float64], r0: float, kmax: int, maxlength: int, rstep: float = 0.01
...
def __r1r2_combinations__(r0: float, rstep: float) -> NDArray[float64]:
    total: int = ceil((1 - r0 - rstep) * (1 - r0) / 2 / (rstep)**2)
    result: NDArray[float64] = empty(shape=(total, 2), dtype=float64)
    idx: int = 0
    for r2 in range(r0, 1 - rstep, rstep):
        for r1 in range(r2 - r0, r2, rstep):
            result[idx, 0] = r1
            result[idx, 1] = r2
            idx += 1
