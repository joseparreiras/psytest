...
from psytest.utils.constants import KMAX, TEST_SIZE
...
def bsadfuller_critval(
    ...
    testsize: list[float] | float = [0.10, 0.05, 0.01],
) -> NDArray[float64]:
    """
    ...
    Args:
        ...
        testsize (list[float] | float, optional): Significance levels to use for the critical values. Defaults to [0.10, 0.05, 0.01].
    ...
    if isinstance(testsize, float):
        quant: float = 1 - testsize
    elif isinstance(testsize, Iterable):
        quant: list[float] = [1 - q for q in testsize]
    critval: NDArray[float64] = quantile(sadf_dist, quant, axis=0)
    return critval
