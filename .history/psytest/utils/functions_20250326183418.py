...
@njit(parallel=True)
def adf_asympt_cdf(wiener: np.ndarray, eval_grid: ArrayLike[no.float64]):
    cdf: np.ndarray = np.zeros(len(eval_grid))
    for i in prange(len(eval_grid)):
        r2: np.float64 = eval_grid[i]
        num: float = 