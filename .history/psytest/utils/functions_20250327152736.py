from numpy import ceil, sqrt, cumsum, float64, zeros, append
...
@njit(parallel=True)
def random_walk(nobs: int) -> NDArray[float64]:
    """
    Generates a simulation of a random walk with size `nobs`.

    Args:
        nobs (int): Number of observations.

    Returns:
        NDArray[float64]: The random
    """
    y: NDArray[float64] = zeros(nobs)
    for i in prange(1, nobs):
        y[i] = y[i - 1] + normal()
    return y


def random_walk(size: tuple[int, int] | int) -> NDArray[float64]:
    """
    Generates a simulation of a random walk with size `size`.

    Args:
        size (tuple[int, int]): The size of the random walk.

    Returns:
        NDArray[float64]: The random walk.
    """
    if isinstance(size, int):
        size = (size, 1)
    return cumsum(normal(size=size), axis=1)


def random_walk_like(x: NDArray) -> NDArray[float64]:
    """
    Simulates a random walk with the same size as the input array.
