...
@njit(parallel=True)
def random_walk(nobs: int, nreps: int) -> NDArray[float64]:
    """
    ...
    errors: NDArray[float64] = array([normal() for _ in prange((nobs-1) * (nreps-1))])
    errors = errors.reshape((nreps-1, nobs-1))
