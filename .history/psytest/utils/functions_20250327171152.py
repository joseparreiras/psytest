from numpy import ceil, cumsum, float64, linspace
...
def random_walk(nreps: int, nobs: int) -> NDArray[float64]:
    ...
    """
    # TODO: Redo this, distribution under the null is not this. Follow eq(3) in the paper
    err: NDArray[float64] = nobs ** (-0.5) * normal(size=(nreps, nobs))
    trend: NDArray[float64] = linspace(0, 1, nobs+1)[1:]
    y = cumsum(err, axis=1) + 1 / trend
    y[0, :] = 0
    return cumsum(err, axis=1) + 1 / trend
    return cumsum(nobs ** (-0.5) * normal(size=(nreps, nobs)), axis=1)
