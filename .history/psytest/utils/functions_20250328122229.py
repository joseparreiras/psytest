...
@njit(parallel=False)
def make_iterable(
    maxs: NDArray[int64], mins: NDArray[int64] | None = None
) -> NDArray[int64]:
    ndim: int = len(maxs)
    if mins is None:
        mins_array: NDArray[int64] = zeros(ndim, dtype=int64)

    iter_size = 0
    for i in range(ndim):
        iter_size += maxs[i] - mins_array[i]
    iter_size = sum([max - min + 1 for max, min in zip(maxs, mins_array)])
    result: NDArray[int64] = zeros((iter_size, ndim), dtype=int64)

    idx: int = 0
    for i in range(ndim):
        for x in range(mins_array[i], maxs[i] + 1):
            result[idx, i] = x
            idx += 1
