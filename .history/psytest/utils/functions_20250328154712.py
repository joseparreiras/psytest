from numpy import ceil, cumsum, float64, zeros, int64, repeat
...
def simulate_markov(nobs: int):
    err: NDArray[float64] = normal(size=nobs-1)
    y: NDArray[float64] = zeros(nobs)
    beta_list: list[float] = [1, 0.95]
    beta: NDArray[float64] = repeat(min(beta_list), nobs)
    p: float = 0.80
    for t in range(1, nobs):
        change_regime: bool = uniform() < 1-p
        if change_regime:
            cur_beta = beta_list.index(beta[t-1])
            beta[t] = beta_list[1-cur_beta]
        else:
            beta[t] = beta[t-1]
