...
def simulate_markov(
    ...
    """
    Simulates a markov process with two regimes. The two regimes differ in their beta parameter, one potentially explosive (beta > 1) and the other stationary (beta < 1). The markov matrix is set as constant with the parameter `p` giving the probability of remaining in the same regime. The process is simulated using a normal distribution for the error term.
    ...
    """
    if not isinstance(beta_list, Iterator):
        raise ValueError("beta_list must be a list")
    if len(beta_list) != 2:
