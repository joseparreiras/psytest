# 1. Load the modules
import numpy as np
from psytest import *

# 2. Read the data (Markov switching DGP)

nobs: int = 1000
betas: list[float] = [0.5, 1.0]
p_matrix: np.ndarray = np.array([[0.9, 0.1], [0.1, 0.9]])
test_error: np.ndarray = np.random.normal(size=nobs)


def simulate_markov_y(
    nobs: int, betas: np.ndarray, p_matrix: np.ndarray, test_error: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    y: np.ndarray = np.zeros(nobs)
    beta_array: np.ndarray = np.zeros(nobs)
    beta_array[0] = betas[0]

    for t in range(1, nobs):
        last_beta = beta_array[t - 1]
        idx = 0 if last_beta == betas[0] else 1
        beta_t = np.random.choice(betas, p=p_matrix[idx])
        beta_array[t] = beta_t
        y[t] = beta_t * y[t - 1] + test_error[t]

    return y, beta_array


y, beta_array = simulate_markov_y(nobs, np.array(betas), p_matrix, test_error)
r0 = 10
nreps = 1000

adf = ADFtest(y)
adf.teststat()
adf.critval()
sadf = SADFtest(y, kmax = 0, r0 = None)
sadf.teststat()
sadf.critval()

%timeit -n 1 sadfuller_dist(1000, 100, 10)
%timeit -n 1 sadfuller_dist_numba(1000, 100, 10)

%timeit max(1,2)
%timeit np.max((1,2))

%timeit -n 1 list(__make_iterator__(1000, 1000, 10))


nreps = list(range(4))
r2 = list(range(3))
r1= list(range(2))

%timeit list(__make_iterator__(100, 100, 10))

nobs = 1000
nreps = 1000
%timeit generate_combinations(100, 100, 10)

%timeit sadfuller_dist(1000, 100, 10)

